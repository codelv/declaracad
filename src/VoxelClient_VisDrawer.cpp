...
VoxelClient_VisDrawer::VisElement::~VisElement() { delete myHandler; }
...
void VoxelClient_VisDrawer::VisElement::Render(const Handle(OpenGl_Workspace) &
                                               theWorkspace) const {
  const Standard_Boolean aHl =
      Standard_False; //(theWorkspace->NamedStatus & OPENGL_NS_HIGHLIGHT);
...
//           driver's structure
//=======================================================================
void VoxelClient_PrsGl::Compute(const Handle(PrsMgr_PresentationManager) &
                                    thePrsMgr,
  ...
  Handle(OpenGl_Group) aGroup =
      Handle(OpenGl_Group)::DownCast(thePrs->CurrentGroup());

  Voxel_VisData *data = static_cast<Voxel_VisData *>(myVisData);
  VoxelClient_VisDrawer::VisElement *anElem =
      new VoxelClient_VisDrawer::VisElement(data);
  aGroup->AddElement(anElem);

  Handle(Graphic3d_AspectMarker3d) insideAspect = new Graphic3d_AspectMarker3d(
      Aspect_TOM_POINT, data->myDisplay.myColor, data->myDisplay.myPointSize);

  aGroup->SetPrimitivesAspect(insideAspect);
  thePrs->CurrentGroup()->AddPrimitiveArray(anElem->myHandler->Voxels());

  Graphic3d_BndBox4f aMinMax;
  anElem->EvaluateBounds(aMinMax);
  aGroup->SetMinMaxValues(aMinMax.CornerMin().x(), aMinMax.CornerMin().y(),
                          aMinMax.CornerMin().z(), aMinMax.CornerMax().x(),
                          aMinMax.CornerMax().y(), aMinMax.CornerMax().z());
  thePrsMgr->StructureManager()->Update(Graphic3d_ZLayerId_Top);
}

/**************************************************************************/
VoxelClient_VisDrawer::VoxelClient_VisDrawer(Voxel_VisData *theData)
    : myData(theData) {

  if (Voxel_BoolDS *voxels = myData->myBoolVoxels) {
    Standard_Real xc, yc, zc;
    Standard_Integer nbx = voxels->GetNbX();
    Standard_Integer nby = voxels->GetNbY();
    Standard_Integer nbz = voxels->GetNbZ();
    m_insidePoints = new Graphic3d_ArrayOfPoints(nbx * nby * nbz);
    for (Standard_Integer iy = 0; iy < nby; iy++) {
      for (Standard_Integer iz = 0; iz < nbz; iz++) {
        for (Standard_Integer ix = 0; ix < nbx; ix++) {
...
void VoxelClient_VisDrawer::EvalMinMax(Graphic3d_BndBox4f &theMinMax) const {
  ...
  if (myData->myBoolVoxels) {
    ...
    Graphic3d_Vec4 aBoolVoxelsMax(
        RealToShortReal(myData->myBoolVoxels->GetX() +
                        myData->myBoolVoxels->GetXLen()),
        RealToShortReal(myData->myBoolVoxels->GetY() +
                        myData->myBoolVoxels->GetYLen()),
        RealToShortReal(myData->myBoolVoxels->GetZ() +
                        myData->myBoolVoxels->GetZLen()),
        1.0f);
  ...
  if (myData->myColorVoxels) {
    Graphic3d_Vec4 aColorVoxelsMin(
        RealToShortReal(myData->myColorVoxels->GetX()),
        RealToShortReal(myData->myColorVoxels->GetY()),
        RealToShortReal(myData->myColorVoxels->GetZ()), 1.0f);
    Graphic3d_Vec4 aColorVoxelsMax(
        RealToShortReal(myData->myColorVoxels->GetX() +
                        myData->myColorVoxels->GetXLen()),
        RealToShortReal(myData->myColorVoxels->GetY() +
                        myData->myColorVoxels->GetYLen()),
        RealToShortReal(myData->myColorVoxels->GetZ() +
                        myData->myColorVoxels->GetZLen()),
        1.0f);
  ...
  if (myData->myROctBoolVoxels) {
    Graphic3d_Vec4 aROctBoolVoxelsMin(
        RealToShortReal(myData->myROctBoolVoxels->GetX()),
        RealToShortReal(myData->myROctBoolVoxels->GetY()),
        RealToShortReal(myData->myROctBoolVoxels->GetZ()), 1.0f);
    Graphic3d_Vec4 aROctBoolVoxelsMax(
        RealToShortReal(myData->myROctBoolVoxels->GetX() +
                        myData->myROctBoolVoxels->GetXLen()),
        RealToShortReal(myData->myROctBoolVoxels->GetY() +
                        myData->myROctBoolVoxels->GetYLen()),
        RealToShortReal(myData->myROctBoolVoxels->GetZ() +
                        myData->myROctBoolVoxels->GetZLen()),
        1.0f);
  ...
  if (!myData->myTriangulation.IsNull()) {
    Standard_Real x, y, z;
    const auto &nodes = myData->myTriangulation->InternalNodes();
    Standard_Integer inode = nodes.Lower(), nb_nodes = nodes.Upper();
    for (; inode <= nb_nodes; inode++) {
      nodes.Value(inode).Coord(x, y, z);
      Graphic3d_Vec4 aNodeCoord(RealToShortReal(x), RealToShortReal(y),
                                RealToShortReal(z), 1.0f);
...
// COlor
static void setColor(const Quantity_Color &color,
                     const Standard_Boolean highlight) {
  static Quantity_Color highlight_color(Quantity_NOC_BLUE1);
  if (highlight)
    glColor3f((GLfloat)highlight_color.Red(), (GLfloat)highlight_color.Green(),
              (GLfloat)highlight_color.Blue());
  else
...
static void getNormal(gp_Dir &normal) {
  ...
  //   gluUnProject(viewport[0], viewport[1], 0., model_matrix, proj_matrix,
  //                viewport, &x, &y, &z);
  gp_Pnt p1(x, y, z);
  //   gluUnProject(viewport[0] + viewport[2], viewport[1], 0., model_matrix,
  //                proj_matrix, viewport, &x, &y, &z);
  gp_Pnt p2(x, y, z);
  //   gluUnProject(viewport[0], viewport[1] + viewport[3], 0., model_matrix,
  //                proj_matrix, viewport, &x, &y, &z);
  gp_Pnt p3(x, y, z);
...
static void getVoxel3Directions(const gp_Dir &viewnormal, VoxelDirection &vdir1,
                                VoxelDirection &vdir2, VoxelDirection &vdir3) {
  Standard_Boolean vdir1_set = Standard_False, vdir2_set = Standard_False,
                   vdir3_set = Standard_False;
...
CheckSize(Voxel_DS *voxels, const Standard_Integer ix,
          const Standard_Integer iy, const Standard_Integer iz,
          const Standard_Real xmin, const Standard_Real xmax,
          const Standard_Real ymin, const Standard_Real ymax,
          const Standard_Real zmin, const Standard_Real zmax, Standard_Real &xc,
          Standard_Real &yc, Standard_Real &zc) {
  voxels->GetCenter(ix, iy, iz, xc, yc, zc);
  if (xc < xmin || xc > xmax)
...
static Standard_Boolean
CheckSize(Voxel_ROctBoolDS *voxels, const Standard_Integer ix,
          const Standard_Integer iy, const Standard_Integer iz,
...
static void drawBoolPoints(const VoxelDirection vdir,
                           const Standard_Boolean nearest, Voxel_BoolDS *voxels,
                           const Standard_Real xmin, const Standard_Real xmax,
                           const Standard_Real ymin, const Standard_Real ymax,
  ...
  case Xminus: {
    for (iy = 0; iy < nby; iy++) {
      for (iz = 0; iz < nbz; iz++) {
        for (ix = 0; ix < nbx; ix++) {
          if (check_size && !CheckSize(voxels, ix, iy, iz, xmin, xmax, ymin,
                                       ymax, zmin, zmax, xc, yc, zc))
            continue;
...
static void
drawROctBoolPoints(const VoxelDirection vdir, const Standard_Boolean nearest,
                   Voxel_ROctBoolDS *voxels, const Standard_Real xmin,
                   const Standard_Real xmax, const Standard_Real ymin,
                   const Standard_Real ymax, const Standard_Real zmin,
      ...
      for (iz = 0; iz < nbz; iz++) {
        for (ix = 0; ix < nbx; ix++) {
          ...
          case 1: {
            for (i = 0; i < 8; i++) {
              if (check_size &&
                  !CheckSize(voxels, ix, iy, iz, i, -1, xmin, xmax, ymin, ymax,
                             zmin, zmax, xc, yc, zc))
                continue;
              Standard_Boolean value =
                  voxels->Get(ix, iy, iz, i) == Standard_True;
              if (value) {
                if (!check_size)
                  voxels->GetCenter(ix, iy, iz, i, xc, yc, zc);
                glVertex3d(xc, yc, zc);
          ...
          case 2: {
            for (i = 0; i < 8; i++) {
              for (j = 0; j < 8; j++) {
                if (check_size &&
                    !CheckSize(voxels, ix, iy, iz, i, j, xmin, xmax, ymin, ymax,
                               zmin, zmax, xc, yc, zc))
                  continue;
                Standard_Boolean value =
                    voxels->Get(ix, iy, iz, i, j) == Standard_True;
                if (value) {
                  if (!check_size)
                    voxels->GetCenter(ix, iy, iz, i, j, xc, yc, zc);
                  glVertex3d(xc, yc, zc);
  ...
  case Zminus: {
    for (ix = 0; ix < nbx; ix++) {
      for (iy = 0; iy < nby; iy++) {
        for (iz = 0; iz < nbz; iz++) {
          const auto depth = voxels->Deepness(ix, iy, iz);
          switch (depth) {
          case 0: {
            if (check_size && !CheckSize(voxels, ix, iy, iz, xmin, xmax, ymin,
                                         ymax, zmin, zmax, xc, yc, zc))
...
static void
drawColorPoints(const VoxelDirection vdir, const Standard_Boolean nearest,
...
static void
drawBoolQuadrangles(Voxel_BoolDS *voxels, const VoxelDirection vdir,
                    const gp_Dir &viewnormal, const Standard_Boolean nearest,
                    const gp_Pnt &p1, const gp_Pnt &p2, const gp_Pnt &p3,
                    const gp_Pnt &p4, const Standard_Real xmin,
                    const Standard_Real xmax, const Standard_Real ymin,
                    const Standard_Real ymax, const Standard_Real zmin,
                    const Standard_Real zmax) {
  gp_Vec vc;
  ...
  Standard_Integer ix, iy, iz, nbx = voxels->GetNbX(), nby = voxels->GetNbY(),
                               nbz = voxels->GetNbZ();
...
static void
drawROctBoolQuadrangles(Voxel_ROctBoolDS *voxels, const VoxelDirection vdir,
                        const gp_Dir &viewnormal,
                        const Standard_Boolean nearest, const gp_Pnt &p1,
  ...
  Standard_Real xc, yc, zc, xn = 0.0, yn = 0.0, zn = 1.0;
  Standard_Integer ix, iy, iz, nbx = voxels->GetNbX(), nby = voxels->GetNbY(),
                               nbz = voxels->GetNbZ(), i, j, deepness;
        ...
        for (ix = 0; ix < nbx; ix++) {
          ...
          for (i = 0; i < 8; i++) {
            for (j = 0; j < 8; j++) {
              ...
              if (deepness == 0) {
                if (check_size &&
                    !CheckSize(voxels, ix, iy, iz, xmin, xmax, ymin, ymax, zmin,
                               zmax, xc, yc, zc))
                  continue;
              } else if (deepness == 1) {
                if (check_size &&
                    !CheckSize(voxels, ix, iy, iz, i, -1, xmin, xmax, ymin,
                               ymax, zmin, zmax, xc, yc, zc))
                  continue;
              } else if (deepness == 2) {
                if (check_size &&
              ...
              Standard_Boolean value = Standard_False;
              switch (deepness) {
              case 0:
...
static void
drawColorQuadrangles(Voxel_ColorDS *voxels, const VoxelDirection vdir,
                     const gp_Dir &viewnormal, const Standard_Boolean nearest,
                     const Handle(Quantity_HArray1OfColor) & hcolors,
                     const gp_Pnt &p1, const gp_Pnt &p2, const gp_Pnt &p3,
                     const gp_Pnt &p4, const Standard_Byte minvalue,
                     const Standard_Byte maxvalue, const Standard_Real xmin,
                     const Standard_Real xmax, const Standard_Real ymin,
                     const Standard_Real ymax, const Standard_Real zmin,
                     const Standard_Real zmax) {
  gp_Vec vc;
...
static void setPlaneNormal(const VoxelDirection &dir, const Standard_Real dx,
                           const Standard_Real dy, const Standard_Real dz,
                           gp_Pln &plane, gp_Pnt &p1, gp_Pnt &p2, gp_Pnt &p3,
                           gp_Pnt &p4) {
...
void VoxelClient_VisDrawer::DisplayVoxels(const Standard_Boolean theHighlight) {
  ...
  if (Voxel_BoolDS *voxels = myData->myBoolVoxels) {
    ...
    /*
    Handle (Graphic3d_Group) outsideGroup = Prs->NewGroup();
    Handle (Graphic3d_AspectMarker3d) outsideAspect = new
    Graphic3d_AspectMarker3d ( Aspect_TOM_POINT, Quantity_NOC_BLUE1,
    data->myDisplay.myPointSize/2.0
    );
    outsideGroup->SetPrimitivesAspect ( outsideAspect );
    outsideGroup->AddPrimitiveArray (outsidePoints);
    */
  } else if (Voxel_ROctBoolDS *voxels = myData->myROctBoolVoxels) {
  }
  return;
...
void VoxelClient_VisDrawer::DisplayPoints(const Standard_Boolean nearest) {
  ...
  if (myData->myBoolVoxels) {
    if (nearest || myData->myDisplay.myDegenerateMode) {
      ...
      if (myData->myDisplay.myUsageOfGLlists) {
        ...
        if (myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir1] <
            0) {
          genListIndex(myData->myDisplay
                           .myBoolNearestPointsList[(Standard_Integer)vdir1]);
          glNewList(myData->myDisplay
                        .myBoolNearestPointsList[(Standard_Integer)vdir1],
                    GL_COMPILE);
          drawBoolPoints(vdir1, Standard_True, myData->myBoolVoxels, xmin, xmax,
                         ymin, ymax, zmin, zmax);
          glEndList();
        }
        if (myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir2] <
            0) {
          genListIndex(myData->myDisplay
                           .myBoolNearestPointsList[(Standard_Integer)vdir2]);
          glNewList(myData->myDisplay
                        .myBoolNearestPointsList[(Standard_Integer)vdir2],
                    GL_COMPILE);
          drawBoolPoints(vdir2, Standard_True, myData->myBoolVoxels, xmin, xmax,
                         ymin, ymax, zmin, zmax);
          glEndList();
        }
        if (myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir3] <
            0) {
          genListIndex(myData->myDisplay
                           .myBoolNearestPointsList[(Standard_Integer)vdir3]);
          glNewList(myData->myDisplay
                        .myBoolNearestPointsList[(Standard_Integer)vdir3],
                    GL_COMPILE);
          drawBoolPoints(vdir3, Standard_True, myData->myBoolVoxels, xmin, xmax,
                         ymin, ymax, zmin, zmax);
          glEndList();
        }
        glCallList(
            myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir1]);
        glCallList(
            myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir2]);
        glCallList(
            myData->myDisplay.myBoolNearestPointsList[(Standard_Integer)vdir3]);
      } else {
        drawBoolPoints(vdir1, Standard_True, myData->myBoolVoxels, xmin, xmax,
                       ymin, ymax, zmin, zmax);
        drawBoolPoints(vdir2, Standard_True, myData->myBoolVoxels, xmin, xmax,
      ...
      if (myData->myDisplay.myUsageOfGLlists) {
        if (myData->myDisplay.myBoolPointsFirst) {
          ...
          drawBoolPoints(vdir, Standard_False, myData->myBoolVoxels, xmin, xmax,
                         ymin, ymax, zmin, zmax);
          glEndList();
    ...
    if (nearest || myData->myDisplay.myDegenerateMode) {
      ...
      if (myData->myDisplay.myUsageOfGLlists) {
        ...
        if (myData->myDisplay.myColorNearestPointsFirst) {
          for (Standard_Integer idir = Xminus; idir <= Zplus; idir++) {
            if (myData->myDisplay.myColorNearestPointsList[idir] > 0) {
              glDeleteLists(myData->myDisplay.myColorNearestPointsList[idir],
                            1);
              myData->myDisplay.myColorNearestPointsList[idir] = -1;
            }
          }
        ...
        // Generate GL lists if needed.
        if (myData->myDisplay
                .myColorNearestPointsList[(Standard_Integer)vdir1] < 0) {
          genListIndex(myData->myDisplay
                           .myColorNearestPointsList[(Standard_Integer)vdir1]);
          glNewList(myData->myDisplay
                        .myColorNearestPointsList[(Standard_Integer)vdir1],
                    GL_COMPILE);
          drawColorPoints(vdir1, Standard_True, myData->myColorVoxels,
                          myData->myDisplay.myColors,
                          myData->myDisplay.myColorMinValue,
                          myData->myDisplay.myColorMaxValue, xmin, xmax, ymin,
                          ymax, zmin, zmax);
          glEndList();
        }
        if (myData->myDisplay
                .myColorNearestPointsList[(Standard_Integer)vdir2] < 0) {
          genListIndex(myData->myDisplay
                           .myColorNearestPointsList[(Standard_Integer)vdir2]);
          glNewList(myData->myDisplay
                        .myColorNearestPointsList[(Standard_Integer)vdir2],
                    GL_COMPILE);
          drawColorPoints(vdir2, Standard_True, myData->myColorVoxels,
                          myData->myDisplay.myColors,
                          myData->myDisplay.myColorMinValue,
                          myData->myDisplay.myColorMaxValue, xmin, xmax, ymin,
                          ymax, zmin, zmax);
        ...
        if (myData->myDisplay
                .myColorNearestPointsList[(Standard_Integer)vdir3] < 0) {
          genListIndex(myData->myDisplay
                           .myColorNearestPointsList[(Standard_Integer)vdir3]);
          glNewList(myData->myDisplay
                        .myColorNearestPointsList[(Standard_Integer)vdir3],
                    GL_COMPILE);
          drawColorPoints(vdir3, Standard_True, myData->myColorVoxels,
                          myData->myDisplay.myColors,
                          myData->myDisplay.myColorMinValue,
                          myData->myDisplay.myColorMaxValue, xmin, xmax, ymin,
                          ymax, zmin, zmax);
      ...
      if (myData->myDisplay.myUsageOfGLlists) {
        if (myData->myDisplay.myColorPointsFirst) {
          ...
          VoxelDirection vdir = getVoxelDirection(viewnormal);
          drawColorPoints(vdir, Standard_False, myData->myColorVoxels,
                          myData->myDisplay.myColors,
                          myData->myDisplay.myColorMinValue,
                          myData->myDisplay.myColorMaxValue, xmin, xmax, ymin,
                          ymax, zmin, zmax);
    ...
    if (nearest || myData->myDisplay.myDegenerateMode) {
      ...
      if (myData->myDisplay.myUsageOfGLlists) {
        ...
        if (myData->myDisplay.myROctBoolNearestPointsFirst) {
          for (Standard_Integer idir = Xminus; idir <= Zplus; idir++) {
            if (myData->myDisplay.myROctBoolNearestPointsList[idir] > 0) {
              glDeleteLists(myData->myDisplay.myROctBoolNearestPointsList[idir],
                            1);
              myData->myDisplay.myROctBoolNearestPointsList[idir] = -1;
            }
          }
        ...
        // Generate GL lists if needed.
        if (myData->myDisplay
                .myROctBoolNearestPointsList[(Standard_Integer)vdir1] < 0) {
          genListIndex(
              myData->myDisplay
                  .myROctBoolNearestPointsList[(Standard_Integer)vdir1]);
          glNewList(myData->myDisplay
                        .myROctBoolNearestPointsList[(Standard_Integer)vdir1],
                    GL_COMPILE);
        ...
        if (myData->myDisplay
                .myROctBoolNearestPointsList[(Standard_Integer)vdir2] < 0) {
          genListIndex(
              myData->myDisplay
                  .myROctBoolNearestPointsList[(Standard_Integer)vdir2]);
          glNewList(myData->myDisplay
                        .myROctBoolNearestPointsList[(Standard_Integer)vdir2],
                    GL_COMPILE);
        ...
        if (myData->myDisplay
                .myROctBoolNearestPointsList[(Standard_Integer)vdir3] < 0) {
          genListIndex(
              myData->myDisplay
                  .myROctBoolNearestPointsList[(Standard_Integer)vdir3]);
          glNewList(myData->myDisplay
                        .myROctBoolNearestPointsList[(Standard_Integer)vdir3],
                    GL_COMPILE);
        ...
        drawROctBoolPoints(vdir1, Standard_True, myData->myROctBoolVoxels, xmin,
                           xmax, ymin, ymax, zmin, zmax);
        drawROctBoolPoints(vdir2, Standard_True, myData->myROctBoolVoxels, xmin,
                           xmax, ymin, ymax, zmin, zmax);
        drawROctBoolPoints(vdir3, Standard_True, myData->myROctBoolVoxels, xmin,
                           xmax, ymin, ymax, zmin, zmax);
      }
    } else {
      if (myData->myDisplay.myUsageOfGLlists) {
        ...
        VoxelDirection vdir = getVoxelDirection(viewnormal);
        drawROctBoolPoints(vdir, Standard_False, myData->myROctBoolVoxels, xmin,
                           xmax, ymin, ymax, zmin, zmax);
      }
    }
  }
  ...
  // cout<<"DisplayPoints()"<<" took "<<minutes<<" minutes, "<<seconds<<"
  // seconds"<<endl;
}
...
void VoxelClient_VisDrawer::DisplayBoxes(const Standard_Boolean nearest) {
  ...
  // Boolean boxes
  if (myData->myBoolVoxels && myData->myBoolVoxels->GetNbX() &&
      myData->myBoolVoxels->GetNbY() && myData->myBoolVoxels->GetNbZ()) {
    // Compute size
    Standard_Real dx = myData->myBoolVoxels->GetXLen() /
                       (Standard_Real)myData->myBoolVoxels->GetNbX();
    Standard_Real dy = myData->myBoolVoxels->GetYLen() /
                       (Standard_Real)myData->myBoolVoxels->GetNbY();
    Standard_Real dz = myData->myBoolVoxels->GetZLen() /
                       (Standard_Real)myData->myBoolVoxels->GetNbZ();
    ...
    drawBoolQuadrangles(myData->myBoolVoxels, vdir1, plane1.Axis().Direction(),
                        skin, p11, p12, p13, p14, xmin, xmax, ymin, ymax, zmin,
                        zmax);
    drawBoolQuadrangles(myData->myBoolVoxels, vdir2, plane2.Axis().Direction(),
                        skin, p21, p22, p23, p24, xmin, xmax, ymin, ymax, zmin,
                        zmax);
    drawBoolQuadrangles(myData->myBoolVoxels, vdir3, plane3.Axis().Direction(),
                        skin, p31, p32, p33, p34, xmin, xmax, ymin, ymax, zmin,
                        zmax);
  }
  // Color quadrangles
  else if (myData->myColorVoxels && myData->myColorVoxels->GetNbX() &&
           myData->myColorVoxels->GetNbY() && myData->myColorVoxels->GetNbZ()) {
    // Compute size
    Standard_Real dx = myData->myColorVoxels->GetXLen() /
                       (Standard_Real)myData->myColorVoxels->GetNbX();
    Standard_Real dy = myData->myColorVoxels->GetYLen() /
                       (Standard_Real)myData->myColorVoxels->GetNbY();
    Standard_Real dz = myData->myColorVoxels->GetZLen() /
                       (Standard_Real)myData->myColorVoxels->GetNbZ();
    Standard_Real d = 0.01 * (Standard_Real)myData->myDisplay.myQuadrangleSize;
    dx *= d;
    dy *= d;
    dz *= d;
    ...
    // Display
    Standard_Boolean skin = nearest || myData->myDisplay.myDegenerateMode;
    drawColorQuadrangles(
        myData->myColorVoxels, vdir1, plane1.Axis().Direction(), skin,
        myData->myDisplay.myColors, p11, p12, p13, p14,
        myData->myDisplay.myColorMinValue, myData->myDisplay.myColorMaxValue,
        xmin, xmax, ymin, ymax, zmin, zmax);
    drawColorQuadrangles(
        myData->myColorVoxels, vdir2, plane2.Axis().Direction(), skin,
        myData->myDisplay.myColors, p21, p22, p23, p24,
        myData->myDisplay.myColorMinValue, myData->myDisplay.myColorMaxValue,
        xmin, xmax, ymin, ymax, zmin, zmax);
    drawColorQuadrangles(
        myData->myColorVoxels, vdir3, plane3.Axis().Direction(), skin,
        myData->myDisplay.myColors, p31, p32, p33, p34,
        myData->myDisplay.myColorMinValue, myData->myDisplay.myColorMaxValue,
        xmin, xmax, ymin, ymax, zmin, zmax);
  }
  // Recursive Octree Boolean boxes
  else if (myData->myROctBoolVoxels && myData->myROctBoolVoxels->GetNbX() &&
           myData->myROctBoolVoxels->GetNbY() &&
           myData->myROctBoolVoxels->GetNbZ()) {
    // Compute size
    Standard_Real dx = myData->myROctBoolVoxels->GetXLen() /
                       (Standard_Real)myData->myROctBoolVoxels->GetNbX();
    Standard_Real dy = myData->myROctBoolVoxels->GetYLen() /
                       (Standard_Real)myData->myROctBoolVoxels->GetNbY();
    Standard_Real dz = myData->myROctBoolVoxels->GetZLen() /
                       (Standard_Real)myData->myROctBoolVoxels->GetNbZ();
    Standard_Real d = 0.01 * (Standard_Real)myData->myDisplay.myQuadrangleSize;
    dx *= d;
    dy *= d;
    dz *= d;
    ...
    // Display
    Standard_Boolean skin = nearest || myData->myDisplay.myDegenerateMode;
    drawROctBoolQuadrangles(myData->myROctBoolVoxels, vdir1,
                            plane1.Axis().Direction(), skin, p11, p12, p13, p14,
                            xmin, xmax, ymin, ymax, zmin, zmax);
    drawROctBoolQuadrangles(myData->myROctBoolVoxels, vdir2,
                            plane2.Axis().Direction(), skin, p21, p22, p23, p24,
                            xmin, xmax, ymin, ymax, zmin, zmax);
    drawROctBoolQuadrangles(myData->myROctBoolVoxels, vdir3,
                            plane3.Axis().Direction(), skin, p31, p32, p33, p34,
                            xmin, xmax, ymin, ymax, zmin, zmax);
  }
}

/**************************************************************************/
void VoxelClient_VisDrawer::DisplayTriangulation(
    const Standard_Boolean theHighlight) {
  ...
  const Poly_Array1OfTriangle &triangles =
      myData->myTriangulation->InternalTriangles();
  Standard_Integer itriangle = triangles.Lower(),
                   nb_triangles = triangles.Upper();
...
void VoxelClient_VisDrawer::HighlightVoxel() {
  if (myData && myData->myDisplay.myHighlightx >= 0 &&
    ...
    ds->GetCenter(myData->myDisplay.myHighlightx,
                  ...
                  myData->myDisplay.myHighlightz, xc, yc, zc);
    ...
    Standard_Real x1 = xc - half_voxelx, y1 = yc - half_voxely,
                  z1 = zc - half_voxelz;
    Standard_Real x2 = xc + half_voxelx, y2 = yc + half_voxely,
                  z2 = zc + half_voxelz;
