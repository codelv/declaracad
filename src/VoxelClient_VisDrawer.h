...
protected:
  ...
  Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
          const Handle(Prs3d_Presentation) & thePrs,
          const Standard_Integer theMode) Standard_OVERRIDE;

public:
  DEFINE_STANDARD_RTTI_INLINE(VoxelClient_PrsGl, Voxel_Prs)
};
...
public:
  ...
  Handle(Graphic3d_ArrayOfPoints) Voxels() { return m_insidePoints; }
...
private:
  ...
  DisplayTriangulation(const Standard_Boolean theHighlight);

  Voxel_VisData *myData;
  Handle(Graphic3d_ArrayOfPoints) m_insidePoints;
};
