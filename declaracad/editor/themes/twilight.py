# -----------------------------------------------------------------------------
# Copyright (c) 2017-2022, Nucleic Development Team.
#
# Distributed under the terms of the Modified BSD License.
#
# The full license is in the file LICENSE, distributed with this software.
# -----------------------------------------------------------------------------

TWILIGHT_THEME = {
    "python": {
        "class_name": {"color": "#9B859D", "font-weight": "bold"},
        "comment": {"color": "#5F5A60", "font-style": "italic"},
        "comment_bock": {"color": "#5F5A60", "font-style": "italic"},
        "decorator": {"color": "#7587A6"},
        "default": {
            "background": "#181818",
            "color": "#F8F8F8",
            "font-family": 'Consolas, Monaco,"Lucida Console"',
        },
        "double_quoted_fstring": {},
        "double_quoted_string": {},
        "function_method_name": {"color": "#9B703F", "font-weight": "bold"},
        "highlighted_identifier": {"color": "#CDA869"},
        "identifier": {"color": "#7587A6"},
        "keyword": {"color": "#CDA869"},
        "number": {},
        "single_quoted_fstring": {},
        "single_quoted_string": {},
        "triple_double_quoted_fstring": {},
        "triple_double_quoted_string": {},
        "triple_single_quoted_fstring": {},
        "triple_single_quoted_string": {},
        "unclosed_string": {"border": "#B22518"},
    },
    "settings": {
        "caret": "#F8F8F8",
        "color": "#F8F8F8",
        "name": "twilight",
        "paper": "#181818",
    },
}

TWILIGHT_THEME["enaml"] = TWILIGHT_THEME["python"]
