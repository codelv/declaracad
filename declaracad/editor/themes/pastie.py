# -----------------------------------------------------------------------------
# Copyright (c) 2017-2022, Nucleic Development Team.
#
# Distributed under the terms of the Modified BSD License.
#
# The full license is in the file LICENSE, distributed with this software.
# -----------------------------------------------------------------------------

PASTIE_THEME = {
    "python": {
        "class_name": {"color": "#bb0066", "font-weight": "bold"},
        "comment": {"color": "#888888"},
        "comment_block": {"color": "#888888"},
        "decorator": {"color": "#555555"},
        "double_quoted_fstring": {"background-color": "#fff0f0", "color": "#dd2200"},
        "double_quoted_string": {"background-color": "#fff0f0", "color": "#dd2200"},
        "function_method_name": {"color": "#0066bb", "font-weight": "bold"},
        "highlighted_identifier": {"color": "#003388"},
        "identifier": {"color": "#336699"},
        "keyword": {"color": "#008800", "font-weight": "bold"},
        "number": {"color": "#0000DD", "font-weight": "bold"},
        "single_quoted_fstring": {"background-color": "#fff0f0", "color": "#dd2200"},
        "single_quoted_string": {"background-color": "#fff0f0", "color": "#dd2200"},
        "triple_double_quoted_fstring": {
            "background-color": "#fff0f0",
            "color": "#dd2200",
        },
        "triple_double_quoted_string": {
            "background-color": "#fff0f0",
            "color": "#dd2200",
        },
        "triple_single_quoted_fstring": {
            "background-color": "#fff0f0",
            "color": "#dd2200",
        },
        "triple_single_quoted_string": {
            "background-color": "#fff0f0",
            "color": "#dd2200",
        },
        "unclosed_string": {"background-color": "#e3d2d2", "color": "#a61717"},
    },
    "settings": {
        "caret": "#000000",
        "color": "#000000",
        "name": "pastie",
        "paper": "#ffffff",
    },
}

PASTIE_THEME["enaml"] = PASTIE_THEME["python"]
