...
def format_value(v: float, precision: int, strip: bool = True) -> str:
    """Converts a float to a string with the specified precision. Removes trailing
    zeros and decimal point to simplify generated code.
    ...
    strip: bool
        Whether to remove trailing zeros and '.'
...
def save_to_file(filename: str, movements: list[Movement], device: "Device"):
    """Write to a file
    ...
    # Must match DeviceConfig.PRECISIONS
    if device.config.precision == "integer":
        precision = 0
    elif device.config.precision == "maximum":
        precision = 12
    else:
        precision = int(device.config.precision)
    ...
    with open(filename, "w") as f:
        ...
        for movement in movements:
            ...
            for point in movement.points:
                ...
                f.write(f"{cmd} X{fmt(x)} Y{fmt(y)} Z{fmt(z)}\n")
